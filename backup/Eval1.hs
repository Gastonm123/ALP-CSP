...
instance Evaluator where
  ...
  evalProc defines random p =
    case p of
      ...
      (Sequential q r) ->
        wrapResult
          <$> runSequential defines random q r
          <*> refusalSequential defines random q r
      _ -> error ""
    where
      wrapResult run refusal = EvalResult {run = run, refusal = refusal}
      ignore = EvalResult { run = const p, refusal = const False }
